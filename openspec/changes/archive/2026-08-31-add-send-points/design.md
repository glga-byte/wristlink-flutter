...
Submission persists pending first, requests the same coordinator drain asynchronously, and returns the durable record without waiting for transport or watch acknowledgement. This lets the confirmation flow navigate immediately to queue-backed status while controller refreshes publish later `sending`, `sent`, `pending`, or `failed` transitions.

Foreground startup awaits queue storage, acknowledgement subscription, interrupted-send recovery, background-scheduler setup, and native transport hydration before delivery is eligible. Once those prerequisites are ready, it launches the mandatory startup fallback drain as tracked asynchronous work so a missing acknowledgement cannot hold the app shell on its initialization spinner. Detached drain failures remain observed, and service shutdown coordinates with any active task. App foregrounding, relevant device-directory updates, and a platform background callback also request a drain. A process-wide mutex plus the database claim prevents duplicate attempts from overlapping triggers. A record restored as sending gets a bounded acknowledgement recovery window; if no acknowledgement arrives, it becomes failed with `deliveryOutcomeUnknown` and requires explicit retry. This chooses safety over automatic duplicate navigation/waypoint operations after an ambiguous crash.

The delivery coordinator consumes the acknowledgement gateway's unified event stream. Valid `GarminAcknowledgementReceived` events enter message-id correlation, while `GarminAcknowledgementDiagnostic` events are mapped into `DeliveryDiagnostic` without mutating queue state. Using one subscription preserves event ordering and prevents an early malformed event from being lost while separate filtered listeners are being attached.
