...
- Replace duplicated hardcoded Android and iOS Connect IQ app UUID metadata values with flavor-driven build configuration.
...
- Keep native Garmin bridge code responsible for reading platform metadata, but make the metadata generated from flavor build configuration.
...
- Document the flavor and configuration contract so future Android/iOS Garmin bridge changes keep the UUID centralized.
